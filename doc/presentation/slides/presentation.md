...
# インターン課題の概要の補足（2025 年 9 月追加）

- **課題 1：**  
  野球部の成長管理を目的としたタレントマネジメントシステムの PoC（概念実証）開発。体力測定記録登録、記録の 3 段階の承認フロー（マネージャー → 部員 → コーチ）、記録の時系列表示、部員管理など、指定必須機能を実装。

- **課題 2：**  
  指定された必須機能に加えて、現場の使いやすさを意識した独自機能の提案・実装も求められた。
  チーム全体および各部員の傾向を時系列グラフで表示する機能を自ら考案・設計・実装し、部員育成や指導に役立つ分析ツールとして提供した。

- **課題 3：**  
  情報リテラシーに関するレポートを作成。情報の取り扱いや AI の影響について考察。（本アプリとは直接関係なし）
